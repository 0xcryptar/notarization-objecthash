...
namespace ObjectHashServer.Services.Implementations
{
    public static class ObjectRedactionImplementation
    {
        ...
        {
            return RecursiveRedactDataAndSalts(json.DeepClone(), redactSettings.DeepClone(), salts.IsNullOrEmpty() ? null : salts.DeepClone());
        }

        private static (JToken json, JToken salts) RecursiveRedactDataAndSalts(JToken json, JToken redactSettings, JToken salts = null)
        {
            // evaluate redact settings if they are of type object
            JToken evaluateRedactSettings =
                EvaluateCommandsInRedactSettingsImplementation.EvaluateCommandsInRedactSettings(redactSettings, json);
            
            // ReSharper disable once SwitchStatementMissingSomeCases
            switch (evaluateRedactSettings.Type)
            {
                ...
                case JTokenType.Object:
                    try
                    {
                        return RedactObject((JObject)json, (JObject)evaluateRedactSettings, salts.IsNullOrEmpty() ? null : (JObject)salts);
                    }
                ...
                case JTokenType.Array:
                    try {
                        return RedactArray((JArray)json, (JArray)evaluateRedactSettings, salts.IsNullOrEmpty() ? null : (JArray)salts);
                    }
                    catch(InvalidCastException e)
                    {
        ...
        private static (JToken json, JToken salts) RedactObject(JObject json, JObject redactSettings, JObject salts = null)
        {
